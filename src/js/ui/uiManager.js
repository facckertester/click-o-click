...
class UIManager {
    ...
    updateBlueprints(blueprints, unlockedBlueprints) {
        ...
        const availableBlueprints = this.game.modules.crafting ? 
            this.game.modules.crafting.getAvailableBlueprints() : [];
    ...
    updateActiveCrafts(activeCrafts) {
        ...
        if (!activeCrafts || activeCrafts.length === 0) {
            container.innerHTML = `
                <div class="empty-state">
        ...
        container.innerHTML = activeCrafts.map(craft => {
            ...
            const totalTime = blueprint.timeToCraft * 1000;
            const progress = totalTime > 0 ? 100 - (remaining / totalTime) * 100 : 100;
            ...
            <div class="reforge-layout">
                ...
                <div class="reforge-interface">
                    <div class="item-selection">
                        ...
                        <div id="reforge-items-list" class="items-list"></div>
                    </div>
    ...
    updateReforgeItemsList() {
        ...
        if (!container || !this.game.modules.reforge) return;
    ...
    setupReforgeEventListeners() {
        ...
        const useMagicEssence = document.getElementById('use-magic-essence');
        const targetStat = document.getElementById('target-stat');
        
        if (preserveQuality) {
            preserveQuality.addEventListener('change', () => {
                this.updateReforgeSummary();
            });
        }
        
        if (useMagicEssence) {
            useMagicEssence.addEventListener('change', () => {
                this.updateReforgeSummary();
            });
        }
        
        if (targetStat) {
            targetStat.addEventListener('change', () => {
                this.updateReforgeSummary();
            });
        }
    ...
    executeReforge() {
        if (!this.selectedReforgeItem || !this.game.modules.reforge) return;
            ...
            <div class="shop-layout">
                ...
                <div class="shop-sidebar">
                    ...
                    <div class="price-history">
                        <h4>Изменение цен:</h4>
                        <div id="price-changes">Цены обновляются ежедневно</div>
                    </div>
                </div>
            </div>
        `;
    ...
    setupShopTabs() {
        document.querySelectorAll('.shop-tab').forEach(tab => {
            tab.addEventListener('click', (e) => {
                ...
                const tabContent = document.getElementById(`shop-${tabId}-tab`);
                if (tabContent) {
    ...
    updateShowroomTab() {
        ...
        const showroomItems = this.game.forge.showroom ? 
            this.game.forge.showroom.map(id => 
    ...
    setupShopEventListeners() {
        ...
        if (processSalesBtn) {
            processSalesBtn.addEventListener('click', () => {
                if (this.game.modules.trading) {
    ...
    loadOrdersScreen() {
        ...
        const clearBtn = document.getElementById('clear-expired-btn');
        if (clearBtn) {
    ...
    updateOrdersList() {
        ...
        if (!container || !this.game.modules.orders) return;
        ...
        if (activeOrders.length === 0) {
            ...
            const forceBtn = document.getElementById('force-generate-orders');
            if (forceBtn) {
    ...
    formatOrderRequirements(order) {
        ...
        for (const [statName, statReq] of Object.entries(requirements.requiredStats || {})) {
            ...
            if (statReq.min !== null && statReq.min !== undefined) reqText += ` ≥ ${statReq.min}`;
            if (statReq.max !== null && statReq.max !== undefined) reqText += ` ≤ ${statReq.max}`;
            
            items.push(`<span class="requirement-item"><i class="fas fa-chart-line"></i> ${reqText}</span>`);
        }
        
        // Зачарования
        if (requirements.requiredEnchantments && requirements.requiredEnchantments.length > 0) {
    ...
    formatOrderReward(reward) {
        ...
        // Золото
        items.push(`<span class="reward-item gold"><i class="fas fa-coins"></i> ${reward.gold || 0} золота</span>`);
    ...
    showCompleteOrderModal(orderId) {
        ...
        if (!order || !this.game.modules.orders) return;
        ...
        if (confirmBtn) {
            confirmBtn.addEventListener('click', () => {
                ...
                if (selectedItemId && this.game.modules.orders) {
    ...
    updateItems(items) {
        ...
        document.querySelectorAll('.sell-item-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const itemId = e.currentTarget.dataset.itemId;
                if (this.game.modules.inventory) {
